<!DOCTYPE html>
<html lang="en">

<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Welcome MVC</title>
  <link href="https://cdn.jsdelivr.net/npm/daisyui@5" rel="stylesheet" type="text/css" />
  <script src="https://cdn.jsdelivr.net/npm/@tailwindcss/browser@4"></script>
</head>

<body>
  <div class="hero bg-base-200 min-h-screen">
    <div class="hero-content text-center">
      <div class="max-w-md">
        <h1 class="text-5xl font-bold">MVC</h1>
        <h3 class="mb-10">MOdel View Controller</h3>
        <a href="index.php?action=add" class="btnAdd btn btn-success my-4">
          <svg xmlns="http://www.w3.org/2000/svg" class="size-6" fill="currrentcolor" viewBox="0 0 256 256">
            <path
              d="M224,128a8,8,0,0,1-8,8H136v80a8,8,0,0,1-16,0V136H40a8,8,0,0,1,0-16h80V40a8,8,0,0,1,16,0v80h80A8,8,0,0,1,224,128Z">
            </path>
          </svg>
          Add Pokemon
        </a>
        <div class="overflow-x-auto h-96 w-96">
          <table class="table table-zebra table-xs table-pin-rows table-pin-cols">
            <thead>
              <tr>
                <th>ID</th>
                <td>Name</td>
                <td>Type</td>
                <td>Access</td>
                <th></th>
              </tr>
            </thead>
            <tbody>
              <?php foreach ($data as $p): ?>
                <tr>
                  <th><?= htmlspecialchars($p['id']) ?></th>
                  <td><?= htmlspecialchars($p['name']) ?></td>
                  <td>
                    <?php if ($p['type'] == 'Water'): ?>
                      <span class="badge badge-outline badge-info">Water</span>
                    <?php elseif ($p['type'] == 'Grass'): ?>
                      <span class="badge badge-outline badge-success">Grass</span>
                    <?php elseif ($p['type'] == 'Fire'): ?>
                      <span class="badge badge-outline badge-error">Fire</span>
                    <?php elseif ($p['type'] == 'Electric'): ?>
                      <span class="badge badge-outline badge-warning">Electric</span>
                    <?php elseif ($p['type'] == 'Normal'): ?>
                      <span class="badge badge-outline badge-secondary">Normal</span>
                    <?php elseif ($p['type'] == 'Poison'): ?>
                      <span class="badge badge-outline badge-primary">Poison</span>
                    <?php elseif ($p['type'] == 'Ghost'): ?>
                      <span class="badge badge-outline badge-accent">Ghost</span>
                    <?php elseif ($p['type'] == 'Dragon'): ?>
                      <span class="badge badge-outline badge-secondary  ">Dragon</span>
                    <?php elseif ($p['type'] == 'Rock'): ?>
                      <span class="badge badge-outline badge-neutral">Rock</span>
                    <?php endif; ?>
                  </td>
                  <td>

                    <!-- Ver (lupa) -->
                    <a href="index.php?action=show&id=<?= $p['id'] ?>" class="btn btn-xs btn-neutral">
                      <svg xmlns="http://www.w3.org/2000/svg" class="size-4" fill="currentcolor" viewBox="0 0 256 256">
                        <path
                          d="M229.66,218.34l-50.07-50.06a88.11,88.11,0,1,0-11.31,11.31l50.06,50.07a8,8,0,0,0,11.32-11.32ZM40,112a72,72,0,1,1,72,72A72.08,72.08,0,0,1,40,112Z">
                        </path>
                      </svg>
                    </a>
                    <a href="index.php?action=edit&id=<?= $p['id'] ?>" class="btn btn-xs btn-neutral">
                      <svg xmlns="http://www.w3.org/2000/svg" class="size-4" fill="currentcolor" viewBox="0 0 256 256">
                        <path
                          d="M227.31,73.37,182.63,28.68a16,16,0,0,0-22.63,0L36.69,152A15.86,15.86,0,0,0,32,163.31V208a16,16,0,0,0,16,16H92.69A15.86,15.86,0,0,0,104,219.31L227.31,96a16,16,0,0,0,0-22.63ZM92.69,208H48V163.31l88-88L180.69,120ZM192,108.68,147.31,64l24-24L216,84.68Z">
                        </path>
                      </svg>
                    </a>
                    <button class="btn btn-xs btn-error deleteBtn" data-id="<?= $p['id'] ?>"
                      data-name="<?= htmlspecialchars($p['name']) ?>">
                      <svg xmlns="http://www.w3.org/2000/svg" class="size-4" fill="currentcolor" viewBox="0 0 256 256">
                        <path
                          d="M216,48H40a8,8,0,0,0,0,16h8V208a16,16,0,0,0,16,16H192a16,16,0,0,0,16-16V64h8a8,8,0,0,0,0-16ZM192,208H64V64H192ZM80,24a8,8,0,0,1,8-8h80a8,8,0,0,1,0,16H88A8,8,0,0,1,80,24Z">
                        </path>
                      </svg>
                    </button>
                  </td>
                </tr>
              <?php endforeach; ?>
            </tbody>
          </table>
        </div>
      </div>
    </div>
  </div>
<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>

<script>
  document.querySelectorAll('.deleteBtn').forEach(btn => {

    btn.addEventListener('click', function () {
      let id = this.getAttribute('data-id');
      let name = this.getAttribute('data-name');

      Swal.fire({
        title: "Delete Pokémon?",
        html: `<b>${name}</b> will be permanently deleted.`,
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#d33",
        cancelButtonColor: "#3085d6",
        confirmButtonText: "Yes, delete",
        cancelButtonText: "Cancel"
      }).then((result) => {
        if (result.isConfirmed) {
          window.location.href = `index.php?action=delete&id=${id}`;
        }
      });
    });

  });
</script>

</body>

</html>